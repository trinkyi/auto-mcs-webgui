...
# Install virtual X server, lightweight window manager, VNC server, and utilities
RUN apt-get update && \
...
# Create entrypoint script to launch virtual X server, VNC server, and auto-mcs GUI
RUN printf '#!/bin/bash\n'                                          > /start.sh && \
